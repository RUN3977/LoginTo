...
import {
  createMobileCameraCaptureSession,
  getMobileCaptureLogPath,
  readMobileCaptureSessions
...
const mobileFileStorage = await import("../src/file-vault-storage.ts");
const pairingClient = await import("../src/pairing-client.ts");
const mobileRuntime = await import("../src/runtime.ts");
const mobileRuntimeState = await import("../src/runtime-state-storage.ts");
const mobileView = await import("../src/view-state.ts");
const deviceContainer = await import("../src/device-container.ts");

const __dirname = fileURLToPath(new URL(".", import.meta.url));
export const workspaceRoot = normalize(join(__dirname, "..", "..", ".."));
export const mobileRoot = normalize(join(__dirname, ".."));
export const publicRoot = join(mobileRoot, "prototype");
export const defaultVaultPath = join(workspaceRoot, ".tmp", "mobile-shell-preview.vault-snapshot.json");
export const defaultRuntimeStatePath = mobileRuntimeState.createDefaultMobileRuntimeStatePath(defaultVaultPath);
const defaultSyncReceiptPath = join(workspaceRoot, ".tmp", "mobile-sync-receipts.json");
...
const ids = {
  ...
  nextId(prefix) {
    this.value += 1;
    return `${prefix}_mobile_${Date.now().toString(36)}_${this.value}`;
  }
};
let mobileShellRuntimePromise;
...
const seedRecords = [
    ...
    reminderDrafts: [
      {
        ...
        message: "Coffee Membership will expire soon",
        daysBefore: 7
      }
    ],
    favorite: true
  },
  {
    ...
    title: "Apple ID",
    values: {
      username: "primary@icloud.com",
      password: "apple-secret-2026",
      url: "https://appleid.apple.com",
      notes: "Critical recovery fields require second unlock."
    }
  },
  {
    type: "bank_card",
    title: "Travel Bank Card",
    values: {
      cardholder: "Zhang",
      card_number: "6225 8800 0000 0826",
      bank_name: "Travel Bank",
      statement_day: "25",
      notes: "Monthly bill reminder is enabled. CVV is not stored by default."
    },
    reminderDrafts: [
      {
        dueAt: "2026-06-25T09:00:00.000Z",
        message: "Travel Bank Card statement day reminder",
        daysBefore: 0
      }
    ]
  }
...
export async function createMobileShellAppState() {
  ...
  await hydrateMobileNotesPreview(runtime, viewState);
  const dueNotifications = await runtime.collectDueReminderNotifications(now());
  const capturePreview = createCapturePreview(runtime);
  const pairingPreview = createMobilePairingScanPreview();
  ...
  const captureSessions = await readMobileCaptureSessions();
  const discovery = await createMobileShellNearFieldDiscoveryFromRuntime(runtime);
  ...
  return {
    ...
    stage: "M1 core usable, mobile UI shell preview",
    deviceContainer: deviceContainer.createDeviceContainerProfile("phone"),
    runtime: runtime.snapshot(now()),
    security: runtime.security.snapshot(now()),
    viewState,
    dueNotifications,
    notificationState,
    reminderCenter: createMobileReminderCenter(viewState, dueNotifications, notificationState),
    capturePreview: {
      draftId: capturePreview.ocrDraft.id,
      source: capturePreview.ocrDraft.source,
      imageAttachmentId: capturePreview.ocrDraft.imageAttachmentId,
      image: {
        encryptedBlobPath: capturePreview.image.encryptedBlobPath,
        mimeType: capturePreview.image.mimeType,
        digest: capturePreview.image.digest,
        encryptedSize: capturePreview.image.encryptedSize,
        source: capturePreview.image.source
      },
      rawText: capturePreview.ocrDraft.rawText,
      suggestion: capturePreview.ocrDraft.typeSuggestions[0],
      typeSuggestions: capturePreview.ocrDraft.typeSuggestions,
      extractedFields: capturePreview.ocrDraft.extractedFields,
      writeDirectlyToVault: false,
      originalImageKeptAsEncryptedAttachment: true
    },
    cameraCapture: {
      status: captureSessions.length ? "captured" : "ready",
      captureLogPath: getMobileCaptureLogPath(),
      sessions: captureSessions.length,
      lastSession: captureSessions.at(-1)
    },
    pairingPreview,
    syncPanel: {
      discovery,
      ...
      receiptCount: syncReceipts.length,
    ...
    capabilities: [
      "local vault lookup",
      "due reminder popup preview",
      "camera OCR draft preview",
      "encrypted attachment metadata",
      "standard QR pairing scan preview",
      "local-only terminal workflow"
    ]
  };
}

async function hydrateMobileNotesPreview(runtime, viewState) {
...
function createMobileReminderCenter(viewState, dueNotifications, notificationState) {
  ...
  const pending = dueNotifications.map((delivery) => toMobileReminderCenterItem({
  ...
  const upcoming = (viewState.upcomingAlerts ?? [])
    .filter((alert) => {
      const alertId = createMobileReminderAlertId(alert);
      return !dueAlertIds.has(alertId);
    })
    .map((alert) => toMobileReminderCenterItem({
      alertId: createMobileReminderAlertId(alert),
      ...
      status: alert.status,
      source: "upcoming"
    }, deliveryByAlertId.get(createMobileReminderAlertId(alert))));
  const history = notificationState.deliveries
    ...
    .slice(-8)
    .map((delivery) => toMobileReminderCenterItem({
      alertId: delivery.alertId,
      recordId: delivery.recordId,
      reminderId: delivery.reminderId,
      recordTitle: delivery.recordTitle,
  ...
  return {
    filters: [
      { id: "all", label: "全部", count: pending.length + upcoming.length + history.length },
    ...
    upcoming,
    history,
    items: [...pending, ...upcoming, ...history]
  };
}

function toMobileReminderCenterItem(alert, delivery) {
...
function createMobileReminderAlertId(alert) {
...
export async function createMobileShellSyncPreview(input = {}) {
  const runtime = await getMobileShellRuntime();
  await seedRuntime(runtime);
  ...
  await appendMobileRecordSyncChanges(runtime, input.requestedAt ?? now());
  await runtime.saveRuntimeState(input.requestedAt ?? now());
  const localSummary = runtime.syncSession.getLocalSummary();
  const localRecords = await summarizeMobileRecordsWithTombstones(runtime);
  ...
  const requestDelivery = await sendSyncRequestToPeer(desktopBaseUrl, {
    ...
    senderBaseUrl: input.senderBaseUrl ?? process.env.LOGINTO_MOBILE_SYNC_BASE_URL ?? "http://127.0.0.1:4177",
    confirmation
  });
  if (!requestDelivery.ok) {
    await markSyncConfirmationFailed(confirmation.id, requestDelivery.deliveredAt);
    await saveFailedMobileSyncRequestReceipt({ runtime, desktopDevice, desktopBaseUrl, confirmation, requestDelivery });
  }
  return {
    ok: true,
    confirmation,
...
export async function receiveMobileShellSyncRequest(input = {}) {
  const runtime = await getMobileShellRuntime();
  await seedRuntime(runtime);
...
export async function createMobileShellNearFieldDiscovery(input = {}) {
  const runtime = await getMobileShellRuntime();
  await seedRuntime(runtime);
  return createMobileShellNearFieldDiscoveryFromRuntime(runtime, input);
}

export async function resolveMobileShellDiscoveryCandidateAction(input = {}) {
  const discovery = input.candidate ? input.discovery : await createMobileShellNearFieldDiscovery(input);
  const candidates = [
    input.candidate,
    ...(input.candidates ?? []),
  ...
  if (candidate.requiresRepairing || candidate.trustStatus === "needs-repairing") {
    return {
      ...
      candidate,
      pairing: createMobilePairingScanPreview(),
...
export function resetMobileShellRuntimeForTests() {
  mobileShellRuntimePromise = undefined;
}

export async function applyMobileShellReminderAction(input = {}) {
  const runtime = await getMobileShellRuntime();
  await seedRuntime(runtime);
  const dueNotifications = await runtime.collectDueReminderNotifications(now());
  ...
  if (!alertId) {
    throw new Error("No mobile reminder notification is available to update");
  }
  ...
  if (action === "complete" || action === "done") {
    ...
    delivery = await runtime.snoozeReminderNotification(
      alertId,
      input.snoozedUntil ?? "2026-06-13T10:41:00.000Z",
      input.at ?? now()
    );
    ...
    throw new Error(`Unsupported mobile reminder action: ${action}`);
  }
...
export async function commitMobileShellOcrDraft(input = {}) {
  const runtime = await getMobileShellRuntime();
  await seedRuntime(runtime);
  const capture = await createCaptureFromLatestSession(runtime, input);
  const acceptedType = input.acceptedType ?? capture.ocrDraft.typeSuggestions[0]?.type ?? "membership";
  const acceptedFieldKeys = input.acceptedFieldKeys
    ?? capture.ocrDraft.extractedFields
      .filter((field) => ["member_name", "member_id", "expires_at", "service_phone"].includes(field.key))
      .map((field) => field.key);
  const record = await runtime.commitOcrCapture({
    capture,
    decision: {
      draftId: capture.ocrDraft.id,
      acceptedType,
      acceptedFieldKeys,
      rejectedFieldKeys: input.rejectedFieldKeys ?? [],
      editedFields: normalizeOcrEditedFields(input.editedFields),
      createReminder: input.createReminder ?? true,
      decidedAt: input.decidedAt ?? now()
    }
  });
  const attachmentNote = `自动整理：原图 ${capture.ocrDraft.imageAttachmentId} 已作为加密附件保存，来源 ${capture.ocrDraft.source ?? "camera"}。`;
  const recordWithAttachmentNote = await runtime.updateRecordFields(record.id, [
    {
      key: "notes",
  ...
  return {
    ok: true,
    record: {
      id: recordWithAttachmentNote.id,
      title: recordWithAttachmentNote.title,
      type: recordWithAttachmentNote.type,
      attachments: recordWithAttachmentNote.attachments.length,
      reminders: recordWithAttachmentNote.reminders.length,
      attachmentNote,
      attachmentIds: recordWithAttachmentNote.attachments.map((attachment) => attachment.id),
      attachmentNames: recordWithAttachmentNote.attachments.map((attachment) => attachment.encryptedBlobPath)
    },
    appState: await createMobileShellAppState()
  };
}

function normalizeOcrEditedFields(editedFields) {
  if (!editedFields || typeof editedFields !== "object" || Array.isArray(editedFields)) {
    return undefined;
  }
...
export async function startMobileShellCameraCapture(input = {}) {
  const session = await createMobileCameraCaptureSession({
    rawText: input.rawText,
    plaintextBase64: input.plaintextBase64,
    mimeType: input.mimeType,
    source: input.source ?? "camera",
    capturedAt: input.capturedAt ?? now(),
    captureDir: input.captureDir,
    captureLogPath: input.captureLogPath
  });
  return {
    ok: true,
    session,
...
export async function createMobileShellRecord(input = {}) {
  const runtime = await getMobileShellRuntime();
  await seedRuntime(runtime);
  const type = input.type ?? "account";
  const title = input.title?.trim() || "新记录";
  const record = await runtime.createRecord({
    draft: vault.createRecordDraft({
      type,
      title,
      values: createRecordValues(type, {
        ...input,
        ...input.values,
        title,
        notes: input.notes
      }),
      reminderDrafts: createReminderDrafts(input, title)
    })
  });
  if (input.attachment) {
...
export async function updateMobileShellRecord(input = {}) {
  const runtime = await getMobileShellRuntime();
  await seedRuntime(runtime);
  const record = findMobileRecord(runtime, input.recordId);
  if (input.title?.trim()) {
...
export async function deleteMobileShellRecord(input = {}) {
  const runtime = await getMobileShellRuntime();
  await seedRuntime(runtime);
  const record = findMobileRecord(runtime, input.recordId);
  await appendDeletedRecordTombstone(toSafeRecordSummary(record), input.deletedAt ?? now());
...
export async function removeMobileShellAttachment(input = {}) {
  const runtime = await getMobileShellRuntime();
  await seedRuntime(runtime);
  const record = findMobileRecord(runtime, input.recordId);
  const attachmentId = String(input.attachmentId ?? "").trim();
  if (!attachmentId) {
    throw new Error("Mobile attachment id is required");
  }
...
export function scanMobileShellPairingPreview(input = {}) {
  const pairingPreview = createMobilePairingScanPreview();
  const payloadText = input.payloadText ?? pairingPreview.qrPayloadText;
  const scannedTarget = input.matrix
    ? pairingClient.scanDesktopPairingMatrix({
        matrix: input.matrix,
        now: input.now ?? now()
      })
    : pairingClient.scanDesktopPairingQr({
        payloadText,
        now: input.now ?? now()
      });
  requireLocalPeerBaseUrl(scannedTarget.descriptor.baseUrl, "pairing peer");

  return {
    ok: true,
    deviceName: scannedTarget.deviceName,
    deviceId: scannedTarget.pairingPayload.device.id,
    publicKeyBase64: scannedTarget.pairingPayload.device.publicKeyBase64,
    endpoint: scannedTarget.descriptor.baseUrl,
    expiresAt: scannedTarget.expiresAt,
    qrPayloadText: input.matrix ? undefined : payloadText,
    previewVerificationCode: input.matrix
      ? pairingPreview.verificationCode
      : createPreviewVerificationCode(JSON.stringify(scannedTarget.pairingPayload))
  };
}

export async function trustMobileShellPairingPreview(input = {}) {
  const runtime = await getMobileShellRuntime();
  const scanned = scanMobileShellPairingPreview(input);
  const verificationCodes = createMobilePairingVerificationCodes(input);
  if (!input.confirmedCode) {
    throw new Error("Mobile pairing requires the six-digit verification code");
  }
  if (!verificationCodes.includes(input.confirmedCode)) {
    throw new Error("Mobile pairing verification code does not match");
  }
  const trustedDevice = runtime.syncSession.trustDevice({
    id: scanned.deviceId,
    name: scanned.deviceName,
    kind: "desktop",
...
export async function revokeMobileShellTrustedDevice(input = {}) {
  const runtime = await getMobileShellRuntime();
  await seedRuntime(runtime);
...
function createMobilePairingVerificationCodes(input = {}) {
  const pairingPreview = createMobilePairingScanPreview();
  const payloadText = input.payloadText ?? pairingPreview.qrPayloadText;
  const payload = sync.decodePairingPayloadText(payloadText);
  return [
    createPreviewVerificationCode(payloadText),
    createPreviewVerificationCode(JSON.stringify(payload)),
    createUnsignedPreviewVerificationCode(payloadText),
    createUnsignedPreviewVerificationCode(JSON.stringify(payload))
  ];
}

export async function pushMobileShellSyncToDesktop(input = {}) {
  const runtime = await getMobileShellRuntime();
  await seedRuntime(runtime);
  const desktopBaseUrl = requireLocalPeerBaseUrl(input.desktopBaseUrl ?? process.env.LOGINTO_DESKTOP_SYNC_BASE_URL ?? "http://127.0.0.1:4173", "desktop sync target");
  const discoveredDesktop = await discoverDesktopDevice(desktopBaseUrl);
  ...
  await assertSyncConfirmationStillCurrent({
    confirmation,
    localRecords: await summarizeMobileRecordsWithTombstones(runtime),
    remoteRecords: (await fetchRemoteSyncSummary(desktopBaseUrl))?.records ?? []
  });
  await appendMobileRecordSyncChanges(runtime, input.syncedAt ?? now());
  ...
  try {
    ...
    await saveFailedMobileOutgoingSyncReceipt({
      runtime,
      desktopDevice,
      desktopBaseUrl,
      exchangePackage,
  ...
  const outgoingReceipt = {
    id: ids.nextId("mobile_sync_receipt"),
    direction: "outgoing",
    status: "success",
    syncedAt: input.syncedAt ?? now(),
  ...
  const outgoingReceipts = await loadSyncReceipts();
  outgoingReceipts.push(outgoingReceipt);
  await saveSyncReceipts(outgoingReceipts.slice(-20));
  return {
    ok: true,
    desktopBaseUrl,
    packageId: exchangePackage.packageId,
    ...
    mobileReceipt: outgoingReceipt,
    outgoingReceipt,
    appState: await createMobileShellAppState()
  };
}

export async function simulateMobileShellSyncFailure(input = {}) {
  const runtime = await getMobileShellRuntime();
  await seedRuntime(runtime);
...
export async function actOnMobileShellSyncConfirmation(input = {}) {
  const runtime = await getMobileShellRuntime();
  await seedRuntime(runtime);
...
export async function receiveMobileShellSyncRequestResult(input = {}) {
  const runtime = await getMobileShellRuntime();
  await seedRuntime(runtime);
  const action = input.action === "confirm" ? "confirm" : input.action === "timeout" ? "timeout" : "reject";
  const actedAt = input.actedAt ?? now();
  ...
  if (action === "confirm") {
    ...
    const autoSync = await pushMobileShellSyncToDesktop({
      ...input,
      confirmationId: confirmation.id,
...
async function saveFailedMobileOutgoingSyncReceipt(input) {
  const failedReceipt = {
    id: ids.nextId("mobile_sync_receipt"),
    direction: "outgoing",
    status: "failure",
...
async function saveFailedMobileSyncRequestReceipt(input) {
  const failedReceipt = {
    id: ids.nextId("mobile_sync_receipt"),
    direction: "outgoing",
    status: "failure",
...
export async function getMobileShellSyncSummary(input = {}) {
  const runtime = await getMobileShellRuntime();
  await seedRuntime(runtime);
  await appendMobileRecordSyncChanges(runtime, input.requestedAt ?? now());
  await runtime.saveRuntimeState(input.requestedAt ?? now());
  return {
    ...
    records: await summarizeMobileRecordsWithTombstones(runtime)
  };
}

export async function receiveMobileShellSyncPackage(input = {}) {
  const runtime = await getMobileShellRuntime();
  await seedRuntime(runtime);
  ...
  await appendMobileRecordSyncChanges(runtime, input.receivedAt ?? now());
  ...
  applyRecordSyncPayloadsToMobileVault(runtime, exchangePackage, report, input.decisions ?? []);
  await runtime.saveVaultState();
...
export function createMobileStatusPayload() {
  return {
    product: "LoginTo mobile shell",
    stage: "M1 core usable, mobile UI shell preview",
    workspaceRoot,
    ...
    capabilities: [
      ...
      "camera OCR draft preview",
      "standard QR face-to-face pairing scan preview",
      "local-only terminal workflow"
    ]
  };
}

async function getMobileShellRuntime() {
  if (!mobileShellRuntimePromise) {
    mobileShellRuntimePromise = createMobileShellRuntime();
  }
  return mobileShellRuntimePromise;
}

async function createMobileShellRuntime() {
  const localDevice = await loadLocalDeviceIdentity({
    path: getDeviceIdentityPath(),
    name: "LoginTo Phone Shell",
    kind: "phone",
    legacyPublicKeyBase64: "mobile-shell-public-key"
  });
  const vaultStorage = new mobileFileStorage.MobileFileVaultStorageAdapter(getMobileVaultPath());
  const runtimeStateStorage = new mobileRuntimeState.MobileFileRuntimeStateStorageAdapter(getMobileRuntimeStatePath());
  return mobileRuntime.createMobileRuntime({
    vaultName: "LoginTo Mobile Shell Vault",
    password,
    saltBase64,
    localDevice,
    vaultStorage,
...
function createCapturePreview(runtime) {
  return runtime.startOcrCapture({
    source: "camera",
    image: {
      encryptedBlobPath: "attachments/mobile-airport-lounge.blob",
      mimeType: "image/jpeg",
      digest: "sha256-mobile-airport-lounge",
      encryptedSize: 2048,
      source: "camera"
    },
...
async function createCaptureFromLatestSession(runtime, input = {}) {
  const sessions = await readMobileCaptureSessions(input);
  const latest = sessions.at(-1);
  if (!latest) {
    return createCapturePreview(runtime);
  }
  return runtime.startOcrCapture({
    source: latest.source ?? "camera",
    imageAttachmentId: latest.imageAttachmentId,
    image: latest.image,
    rawText: input.rawText ?? latest.rawText
  });
}
...
async function seedRuntime(runtime) {
  ...
  const deletedTitles = new Set((await loadDeletedRecordTombstones()).map((record) => record.title));
...
function createMobilePairingScanPreview() {
  const desktopDevice = sync.createDeviceIdentity({
    id: "device_desktop_for_mobile_shell",
    name: "Zhang Desktop",
    kind: "desktop",
    publicKeyBase64: "desktop-for-mobile-shell-key",
    now,
    ids
  });
  ...
  const matrix = sync.encodePairingPayloadMatrix(pairingPayload);
  const qr = sync.encodePairingPayloadQr(pairingPayload);
  return {
    scannedDeviceName: desktopDevice.name,
    localEndpoint: pairingPayload.localEndpoint,
...
function findMobileRecord(runtime, recordId) {
  const records = runtime.repository.listRecords();
  const record = records.find((item) => item.id === recordId);
  if (!record) {
    throw new Error("Mobile record is not available");
  }
  return record;
}

function createRecordValues(type, input = {}) {
  const notes = input.notes ?? "";
  if (type === "membership") {
    return {
      member_name: input.member_name ?? input.title ?? "会员",
      member_id: input.member_id ?? input.username ?? "MEMBER-LOCAL",
      expires_at: normalizeDate(input.expires_at) ?? "2027-01-31T00:00:00.000Z",
  ...
  if (type === "identity_document") {
    return {
      ...
      expires_at: normalizeDate(input.expires_at) ?? "2028-01-31T00:00:00.000Z",
...
function createReminderDrafts(input, title) {
  if (!input.reminderAt) {
    return [];
  }
  return [
    {
      dueAt: normalizeDate(input.reminderAt) ?? input.reminderAt,
...
async function createMobileShellNearFieldDiscoveryFromRuntime(runtime, input = {}) {
...
function getMobileVaultPath() {
  return process.env.LOGINTO_MOBILE_SHELL_VAULT_PATH || defaultVaultPath;
}
...
function getSyncReceiptPath() {
  return process.env.LOGINTO_MOBILE_SYNC_RECEIPTS_PATH || defaultSyncReceiptPath;
}

function getDeviceIdentityPath() {
  return process.env.LOGINTO_MOBILE_DEVICE_IDENTITY_PATH || defaultDeviceIdentityPath;
}

function getSyncConfirmationPath() {
  return process.env.LOGINTO_MOBILE_SYNC_CONFIRMATIONS_PATH || defaultSyncConfirmationPath;
}

function getTrustedDeviceRevocationPath() {
  return process.env.LOGINTO_MOBILE_TRUSTED_DEVICE_REVOCATIONS_PATH || defaultTrustedDeviceRevocationPath;
}

function getSyncDeletionPath() {
  return process.env.LOGINTO_MOBILE_SYNC_DELETIONS_PATH || `${getMobileVaultPath()}.sync-deletions.json`;
}
...
async function loadDeletedRecordTombstones() {
  try {
    const vaultSnapshot = JSON.parse(await readFile(getMobileVaultPath(), "utf8"));
    const vaultId = vaultSnapshot.manifest?.vaultId;
    const parsed = JSON.parse(await readFile(getSyncDeletionPath(), "utf8"));
    return Array.isArray(parsed.deletedRecords)
      ? parsed.deletedRecords.filter((record) => record.vaultId === vaultId)
      : [];
  } catch (error) {
    if (error?.code === "ENOENT") {
      return [];
...
async function saveDeletedRecordTombstones(deletedRecords) {
  const path = getSyncDeletionPath();
  await writeJsonFileAtomically(path, { deletedRecords });
}

async function appendDeletedRecordTombstone(record, deletedAt) {
  const deletedRecords = await loadDeletedRecordTombstones();
  const vaultSnapshot = JSON.parse(await readFile(getMobileVaultPath(), "utf8"));
  const tombstone = {
    ...record,
    vaultId: vaultSnapshot.manifest?.vaultId,
    deleted: true,
    archived: true,
    deletedAt,
    updatedAt: deletedAt
  ...
  await saveDeletedRecordTombstones(nextRecords.slice(-100));
  return tombstone;
}

async function appendMobileRecordSyncChanges(runtime, at) {
  ...
  for (const tombstone of await loadDeletedRecordTombstones()) {
    lamport += 1;
    runtime.appendLocalSyncChange(sync.createSyncChange({
      entity: "record",
...
function applyRecordSyncPayloadsToMobileVault(runtime, exchangePackage, report, decisions) {
  ...
  for (const change of report.appliedChanges) {
    ...
    if (record) {
      upsertMobileVaultRecord(runtime, record);
    }
  }
...
function upsertMobileVaultRecord(runtime, record) {
...
function summarizeMobileRecords(runtime) {
  return runtime.repository.listRecords().map(toSafeRecordSummary);
}

async function summarizeMobileRecordsWithTombstones(runtime) {
  return [
    ...summarizeMobileRecords(runtime),
    ...(await loadDeletedRecordTombstones()).map((record) => ({
      ...record,
      deleted: true,
      archived: true
    }))
